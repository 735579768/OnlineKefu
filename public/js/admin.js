...
window.chatconn = function() {
    ...
    socket.on('userleft', function(json) {
        ...
        $('#admin_right .chat_message[data="home"').show();
        chat_content.append(str);
        scrollbot();
    });
    //监听message事件，打印消息信息
    ...
    socket.on('alluser', function(obj) {
    	console.log(obj);
        var str = '';
        for (var a in obj) {
            str += '<li><a data="' + obj[a]['id'] + '"  href="javascript:;">' + obj[a]['name'] + '<span class="fr pdr10">在线</span></a></li>';
        }
        $('#alluser').html(str);
    });
